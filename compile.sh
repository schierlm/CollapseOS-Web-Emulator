...
cp -R tbasic oldshell build/cfs
...
for app in zasm ed memt tbasic basic zmake avra oldshell; do
	echo Building $app
	collapseos/emul/zasm/zasm -o 62 build/cfs <build/cfs/$app/glue.asm >build/cfsbin/$app
done
collapseos/tools/cfspack/cfspack build/cfs build/cfsbin >build/cfs.cfs
echo "Done!"
